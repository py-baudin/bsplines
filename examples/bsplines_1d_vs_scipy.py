...
fig, axes = plt.subplots(
    nrows=2, ncols=2, num="1d-vs-scipy-derivatives", sharex=True, sharey=True
)
for i, ext in enumerate([None, "natural", "clamped", "periodic"]):
    plt.sca(axes.flat[i])
    _scipy = interpolate.make_interp_spline(
        points, data, k=degree, bc_type=ext
    ).derivative(1)(coords)
    plt.plot(coords, _scipy, label=f"scipy k={degree}")
    if ext is not None:
        _bsplines = bsplines.BSpline.prefilter(
            data, degree=degree, extension=ext
        ).derivative(1)(coords)
        plt.plot(coords, _bsplines, ":", label=f"bsplines k={degree}")
    plt.legend(loc="lower right")
    plt.grid(axis="x")
...
fig, axes = plt.subplots(
    nrows=2, ncols=2, num="1d-vs-scipy-derivatives2", sharex=True, sharey=True
)
for i, ext in enumerate([None, "natural", "clamped", "periodic"]):
    plt.sca(axes.flat[i])
    _scipy = interpolate.make_interp_spline(
